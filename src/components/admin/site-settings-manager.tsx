...
export function SiteSettingsManager() {
  ...
  return <div className="p-5 pt-20 md:p-8 lg:pt-8">
    ...
    <section className="mt-6 max-w-4xl space-y-4 border border-white/10 bg-[#161614] p-5">
      <h2 className="text-lg font-medium text-[#c7a66b]">Frequently asked questions</h2>
      {settings.faq.map((item, index) => (
        <div key={index} className="flex items-start gap-3 border-t border-white/10 pt-3">
          <div className="grid flex-1 gap-3 sm:grid-cols-2">
            <input
              value={item.question}
              placeholder="Question"
              onChange={(e) => setSettings({ ...settings, faq: settings.faq.map((f, i) => i === index ? { ...f, question: e.target.value } : f) })}
              className="border border-white/15 bg-transparent px-3 py-2 text-sm text-white outline-none focus:border-[#c7a66b]"
            />
            <textarea
              value={item.answer}
              placeholder="Answer"
              rows={2}
              onChange={(e) => setSettings({ ...settings, faq: settings.faq.map((f, i) => i === index ? { ...f, answer: e.target.value } : f) })}
              className="border border-white/15 bg-transparent px-3 py-2 text-sm text-white outline-none focus:border-[#c7a66b]"
            />
          </div>
          <button
            ...
            onClick={() => setSettings({ ...settings, faq: settings.faq.filter((_, i) => i !== index) })}
            className="mt-2 rounded p-1.5 text-white/40 transition hover:bg-white/5 hover:text-[#e7a29b]"
          >
            <Trash2 size={16} />
          </button>
        </div>
      ))}
      <Button variant="outline" onClick={() => setSettings({ ...settings, faq: [...settings.faq, { question: '', answer: '' }] })}>Add question</Button>
    </section>
